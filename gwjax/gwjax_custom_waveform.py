...
the smallest possible adapter that plugs it into the GWjax workflow
(``Network.project_waveform``, ``Interferometer.inject_signal``,
``make_likelihood_fn``, ``GWjaxNestedSampler``).
...
...     return hp, hc
>>>
>>> wf = gwjax.CustomWaveform(my_waveform, domain="fd",
...
class CustomWaveform:
    """Wrap a user-supplied JAX waveform function for use throughout GWjax.
    ...
    def __call__(self, params: dict, grid_array):
        ...
        :class:`~gwjax.gwjax_samplers.GWjaxNestedSampler`; you can pass
