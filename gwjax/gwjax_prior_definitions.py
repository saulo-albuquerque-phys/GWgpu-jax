"""
gwjax_prior_definitions.py
==========================
Non-uniform prior definitions for GWjax nested-sampling parameter
estimation — fully JAX / ``jit``-traceable, drop-in compatible with the
``blackjax.ns`` machinery already used across the pipeline.

Motivation
----------
The base GWjax samplers draw every sampled parameter from a *plain uniform*
