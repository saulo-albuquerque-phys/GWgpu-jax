"""
gwjax_relative_binning.py
==========================
Relative-binning (Zackay, Dai & Venumadhav 2018, arXiv:1806.08792) log-likelihood
for GWjax, with optional analytical marginalisation over the coalescence time
``tc`` and the coalescence phase ``phi_c``.

This is the GWjax-API counterpart of the SHARPy-era ``relative_binning.py``: it
keeps the same algorithm but builds everything against
:class:`gwjax.Network` / :class:`gwjax.Interferometer` and the GWjax
inner-product convention, so it interoperates with the rest of the toolkit
(``CustomWaveform``, ``GWjaxNestedSampler``, ``gwjax.compat.attach_event_to_network``,
``gwjax.make_likelihood_fn``, …) without any SHARPy dependency.
...
(implemented in :mod:`gwjax.gwjax_likelihood_utils`).  The Gaussian
...
--------------------------
``waveform_fn`` is the same callable used by :class:`gwjax.GWjaxNestedSampler`::
...
:meth:`gwjax.Interferometer.antenna_pattern` and
:meth:`gwjax.Interferometer.time_delay_from_geocenter`.
...
be correct.  See ``examples/gwjax_GW170817_mlgw_bns_jax_rb_analyt_time_phase_marg_pe.ipynb``
...
``GWjaxNestedSampler.loglikelihood_fn`` (sampled keys ∪ fixed_params keys), and
...
from gwjax.gwjax_likelihood_utils import precompute_weight, waveform_projection_fd
...
def _project_at_freqs(
    ...
    Matches the projection used inside :class:`gwjax.GWjaxNestedSampler`::
...
def precompute_rb_network(
    ...
    Parameters
    ----------
    network : gwjax.Network
        Populated with strain data and PSDs on each interferometer
        (see :func:`gwjax.compat.attach_event_to_network`).
    waveform_fn : callable
        ``waveform_fn(params_dict, freqs) -> (hp, hc)`` — same contract as
        :class:`gwjax.GWjaxNestedSampler`.  Must apply ``phi_c`` as a uniform
    ...
    for ifo in ifos:
        ...
        # Per-IFO weight w_k = 4·df·mask/PSD (the GWjax inner-product weight).
...
def build_rb_likelihood(
    ...
    fixed_params : dict, optional
        ...
        dict before each call (same contract as ``GWjaxNestedSampler``).
