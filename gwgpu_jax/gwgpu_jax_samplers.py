"""
gwgpu_jax_samplers.py
==================
Nested-sampling interface for gravitational-wave parameter estimation,
built on top of ``blackjax.ns`` (BlackJAX-NS).

The high-level entry point is :class:`GWgpu_jaxNestedSampler`, which couples:

  * a :class:`~gwgpu_jax.gwgpu_jax_network_ifos_utils.Network` (data + PSDs),
...
...     GWgpu_jaxNestedSampler, build_ripplegw_waveform_fn,
... )
>>> waveform_fn = build_ripplegw_waveform_fn(approximant="IMRPhenomD", f_ref=20.0)
>>> sampler = GWgpu_jaxNestedSampler(
...     network=net,
...
class NestedSamplingResult(NamedTuple):
    """Output bundle from :meth:`GWgpu_jaxNestedSampler.run`."""
...
def build_ripplegw_waveform_fn(
    ...
    def waveform_fn(params, freqs):
        ...
        # (gwgpu_jax_likelihood_utils.waveform_projection_fd) also applies tc via
...
class GWgpu_jaxNestedSampler:
    """Adaptive Nested Slice Sampler bound to a GW data network.

    Parameters
    ----------
    network : Network
        A populated :class:`~gwgpu_jax.gwgpu_jax_network_ifos_utils.Network` with
        strain data and PSDs set on each interferometer.
    waveform_fn : callable or :class:`~gwgpu_jax.gwgpu_jax_custom_waveform.CustomWaveform`
        ``waveform_fn(params, freqs) -> (hp, hc)``. Must be JAX-traceable.
        ``params`` is a dict of scalar JAX values. A
        :class:`~gwgpu_jax.gwgpu_jax_custom_waveform.CustomWaveform` instance
    ...
    priors : dict[str, PriorSpec | str] or None, optional
        ...
        name(s) to either a
        :class:`~gwgpu_jax.gwgpu_jax_prior_definitions.PriorSpec` instance or a
        string alias (``"sin"``, ``"cos"``, ``"volumetric"``, ``"uniform"``;
        see :data:`gwgpu_jax.gwgpu_jax_prior_definitions.PRIOR_ALIASES`). String
    ...
    gmst : float, optional
        ...
        * ``None`` (recommended): use ``network.gmst`` — set automatically
          by :func:`gwgpu_jax.compat.attach_event_to_network` to GMST(trigger),
